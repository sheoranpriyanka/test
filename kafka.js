...
var initiateStateProducer = function(req, res, GatewayData){
  ...
  var kafka = require('kafka-node'),
        ...
        console.log(msg);
        var payloads = [{topic:'initialPhase2', messages:msg}];
...
var initiateStateConsumer = function(){
  var kafka = require('kafka-node'),
    Consumer = kafka.Consumer,
    client = new kafka.Client(),
    consumer = new Consumer(client,
        [{ topic: 'initialPhase2', partition:0}],
        {
            autoCommit: true
        }
    );
...
var secondPhaseProducer = function(arg, tx){
  var kafka = require('kafka-node'),
        ...
        msg = JSON.stringify(msg);
        var payloads = [{topic:"secondPhase2", messages:msg}];
        producer.on('ready', function (err) {
...
var secondePhaseConsumer = function(req, res){
  var kafka = require('kafka-node'),
    ...
    client = new kafka.Client(),
    consumer = new Consumer(client,
        [{ topic: 'secondPhase2', partition:0}],
        {
            autoCommit: true
        }
    );
...
consumer.on('message', function (message) {
    console.log(message);
    //res.send(message);
    transcModel.saveTranscation(req, res, message);
});
